...
const Feed = () => {
  ...
  const fetchFeedPage = async (page = 1) => {
    try {
      const res = await axios.get(`${BASE_URL}/feed?page=${page}&limit=1`, { withCredentials: true });
  ...
  if (users?.length === 0 && !hasMore) {
    return (
      <div className="min-h-screen flex justify-center items-center bg-slate-200">
        <h1 className="text-lg font-semibold text-gray-800">No new users found!</h1>
      </div>
    );
  }
  ...
  return (
    <div className="flex-grow flex justify-center items-center py-8 bg-slate-200 min-h-screen">
      ...
      (users && users[currentUserIndex] ? (
        ...
        <h1 className="text-lg font-semibold text-gray-800">You have interacted with all the profiles.</h1>
      ))}
    </div>
  );
};
